...
import { ArrowLeft, CheckCircle2, ChevronRight, Clock3, MapPin, ShieldCheck } from "lucide-react";
    ...
    return (
      <main className="min-h-screen bg-[#EEECE5] px-4 py-5 text-[#172044]">
        <div className="mx-auto min-h-[calc(100vh-40px)] max-w-[520px] rounded-[28px] bg-[#FBF9F4] p-5 sm:p-8">
          ...
          <div className="mt-20 text-center"><h1 className="text-2xl font-extrabold tracking-[-0.04em]">Report not found</h1><p className="mt-2 text-sm text-[#777C8B]">This report may have been removed or is no longer available.</p></div>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#EEECE5] text-[#172044]">
      <div className="mx-auto min-h-screen max-w-[1280px] bg-[#FBF9F4]">
        <header className="flex items-center justify-between border-b border-[#E5E1D8] px-5 py-4 sm:px-8">
          <Link href="/lost-found" className="inline-flex items-center gap-2 text-[13px] font-semibold text-[#555C70] hover:text-[#5141A9]"><ArrowLeft size={16} /> Lost &amp; Found</Link>
        ...
        <div className="mx-auto grid max-w-5xl lg:grid-cols-[1.08fr_.92fr] lg:gap-10 lg:px-8 lg:py-8">
          <div>
            <div className="relative aspect-[1.08/1] overflow-hidden bg-[#E8E4E0] sm:aspect-[1.35/1] lg:rounded-[24px]">
              <Image src={report.image} alt={report.title} fill priority sizes="(max-width: 1024px) 100vw, 55vw" className="object-cover" />
              <span className="absolute left-4 top-4 rounded-full bg-[#FFFDF9]/95 px-3 py-1.5 text-[9px] font-extrabold uppercase tracking-[0.1em] text-[#29304B] shadow-sm">{report.category}</span>
            </div>
          </div>
          ...
          <div className="px-5 pb-10 pt-6 sm:px-8 lg:px-0 lg:pt-1">
            ...
            <div className="mt-5 grid grid-cols-2 gap-2">
              <div className="rounded-[16px] border border-[#E4E0D8] bg-[#FFFDF9] p-3.5"><MapPin size={16} className="text-[#5D48D2]" /><p className="mt-2 text-[9px] font-bold uppercase tracking-[0.1em] text-[#9294A0]">Location</p><p className="mt-0.5 text-[12px] font-bold text-[#30364E]">{report.location}</p></div>
              <div className="rounded-[16px] border border-[#E4E0D8] bg-[#FFFDF9] p-3.5"><Clock3 size={16} className="text-[#5D48D2]" /><p className="mt-2 text-[9px] font-bold uppercase tracking-[0.1em] text-[#9294A0]">Reported</p><p className="mt-0.5 text-[12px] font-bold text-[#30364E]">{report.time}</p></div>
            </div>

            <section className="mt-7 border-t border-[#E4E0D8] pt-6">
              <h2 className="text-[12px] font-extrabold uppercase tracking-[0.13em]">What happened</h2>
              <p className="mt-2.5 text-[13px] leading-6 text-[#676D7E]">{report.description}</p>
            </section>

            <section className="mt-6 rounded-[18px] border border-[#E3DED4] bg-[#F7F4EC] p-4">
              <div className="flex gap-3"><ShieldCheck size={19} className="mt-0.5 shrink-0 text-[#5D48D2]" /><div><h2 className="text-[12px] font-extrabold">Keep it verifiable</h2><p className="mt-1 text-[11px] leading-5 text-[#74798A]">Only claim this item if you can provide details that are not publicly visible. This helps prevent false claims.</p></div></div>
            </section>

            <div className="mt-6 space-y-2.5">
              <Link href={`/lost-found/${report.id}/claim`} className="flex h-12 w-full items-center justify-center gap-2 rounded-[15px] bg-[#23265B] text-[12px] font-extrabold text-white shadow-[0_8px_20px_rgba(35,38,91,.15)]">{isLost ? "I found this item" : "This is my item"}<ChevronRight size={15} /></Link>
              <Link href="/lost-found" className="flex h-11 w-full items-center justify-center rounded-[15px] border border-[#DEDAD4] bg-[#FFFDF9] text-[12px] font-bold text-[#555B6D]">Back to reports</Link>
            </div>
